...
int main(int argc, char **argv){
    ...
    target.text = get_section_by_name(elf, ".text");
    target.plt_got = get_section_by_name(elf, ".plt.got");
    target.plt_sec = get_section_by_name(elf, ".plt.sec");

    if(*(uint64_t*)(pa + elf->sheader[target.text].sh_offset + 0x750) != 0){
        fprintf(stderr,"This elf was encrypted\n");
        return EXIT_FAILURE;
    }
    munmap(pa, fsize);    
    if(packed_and_mapped_then_write_to_memory(elf, fsize, target, argv[1])){
